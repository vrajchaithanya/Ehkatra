...
pub fn run(path: &str, rows: usize) -> Result<String, String> {
    ...
    // this repo could show what it looks like. The composing text is Latin on
    // purpose — a pinyin IME composes `nihao` before committing 你好, and the
    // bundled font can draw the first and not the second (**TD-79**). Composing
    // kana here would produce a frame full of `.notdef` boxes and prove
    // nothing about the underline.
    click(&mut app, 8, 14);
    app.ime_preedit("nihao", Some((5, 5)));
    let composing = app.frame();
    let ime_path = path.replace(".png", "-ime.png");
    log.push(format!(
        "composing 'nihao' in O9  -> {ime_path} (editor holds {:?}, cell still {}, caret area {:?})",
