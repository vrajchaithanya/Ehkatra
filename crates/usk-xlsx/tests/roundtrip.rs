...
/// Address → everything the model claims about a cell. Session 30 widened this
/// from `(value, formula, number format)` to include the three facets ADR-041
/// added, so **more is compared than in session 29** — the published fidelity
/// number is over a larger surface, which is the honest way to report it.
type CellMap = BTreeMap<(u32, u32), Modelled>;

type Modelled = (
    Value,
    Option<String>,
    Option<String>,
    Option<usk_xlsx::FontFacet>,
    Option<u32>,
    Option<usk_xlsx::Alignment>,
);
...
fn synthetic() -> Workbook {
    ...
    // The ADR-041 facets, at the file boundary. Every one of these has to come
    // back byte-identical through `styles.xml`'s three tables: a font that is
    // only bold, one that is only coloured, one that is 10.5pt (the half-point
    // the encoding exists for), two cells sharing a fill so the dedup path is
    // walked, and each alignment axis on its own.
    let styled = {
        let font = |flags, half_points, argb, name: &str| {
            Some(usk_xlsx::FontFacet {
                flags,
                half_points,
                argb,
        ...
        let align = |horizontal, vertical, wrap| {
        ...
        let styled_cell = |row, col, f, fill, a| Cell {
            row,
            col,
            value: Value::Number(row as f64 + col as f64),
            formula: None,
            number_format: None,
            font: f,
            fill,
        ...
        Sheet {
            name: String::from("Styled"),
            part: String::new(),
            cells: vec![
                styled_cell(
                    0,
                    0,
                    font(usk_xlsx::FONT_BOLD, 22, 0xFF00_0000, "Calibri"),
                    None,
                    None,
                ),
                styled_cell(
                    0,
                    1,
                    font(usk_xlsx::FONT_ITALIC, 22, 0xFF00_0000, "Calibri"),
                    None,
                    None,
                ),
                styled_cell(
                    ...
                    2,
                    font(
                        usk_xlsx::FONT_BOLD | usk_xlsx::FONT_ITALIC | usk_xlsx::FONT_UNDERLINE,
                        28,
                        0xFFC0_0000,
                        "Arial",
                    ),
                ...
                // 10.5pt — the reason the facet stores half-points.
                styled_cell(0, 3, font(0, 21, 0xFF00_0000, "Calibri"), None, None),
                styled_cell(
                    0,
                    4,
                    font(usk_xlsx::FONT_STRIKE, 22, 0xFF00_0000, "Calibri"),
                    None,
                    None,
                ),
                styled_cell(1, 0, None, Some(0xFFFF_FF00), None),
                // Same fill as B2: the table must hold one entry, not two.
                styled_cell(1, 1, None, Some(0xFFFF_FF00), None),
                styled_cell(1, 2, None, Some(0xFF00_80FF), None),
                styled_cell(2, 0, None, None, align(1, 0, false)),
                styled_cell(2, 1, None, None, align(2, 1, false)),
                styled_cell(2, 2, None, None, align(3, 2, true)),
                styled_cell(2, 3, None, None, align(0, 0, true)),
                // All four facets on one cell, which is the case a per-facet
                // model has to get right and a blob model gets right by luck.
                Cell {
                    row: 3,
                    col: 0,
                    ...
                    formula: None,
                    number_format: Some(String::from("0.00%")),
                    font: font(usk_xlsx::FONT_BOLD, 24, 0xFF00_00FF, "Segoe UI"),
                    fill: Some(0xFFEE_EEEE),
                    alignment: align(2, 2, true),
                },
                // A cell that is *only* formatting — XLSX's style-holding cell,
                // and the one the reader used to drop before facets existed.
                Cell {
                    row: 4,
                    col: 0,
                    value: Value::Blank,
    ...
    Workbook {
        sheets: vec![dense, formulas, sparse, styled],
...
fn the_synthetic_workbook_round_trips_at_full_fidelity() {
    ...
    assert_eq!(written.report.cells_written, 48);
    assert_eq!(written.report.formulas_written, 6);
    assert_eq!(written.report.number_formats_written, 6);
    // The ADR-041 surface, counted separately so a regression in one facet is
    // visible rather than absorbed into a total.
    assert_eq!(written.report.fonts_written, 6);
    assert_eq!(written.report.fills_written, 5);
    assert_eq!(written.report.alignments_written, 5);
    assert_eq!(written.report.styles_written, 19);
}

/// `cellXfs` is a flyweight table and must behave like one: two cells with
/// identical formatting name **one** entry, and a fill used twice is stored
/// once. This is ADR-041 decision 4 at the file boundary, and it is what makes
/// a formatted column cheap in the file as well as in memory.
#[test]
fn identical_formatting_shares_one_style_entry() {
    let yellow = |row| Cell {
        row,
        col: 0,
        value: Value::Number(1.0),
    ...
    let book = Workbook {
        sheets: vec![Sheet {
            ...
            part: String::new(),
            cells: (0..50).map(yellow).collect(),
        }],
        fidelity: Fidelity::default(),
    };
    let written = write(&book).expect("write");
    let xml = String::from_utf8(part_bytes(&written, "xl/styles.xml")).expect("utf8");
    assert_eq!(
        xml.matches("<xf ").count(),
        // One `cellStyleXfs` entry, plus `cellXfs` 0 (the default) and the one
        // entry all fifty cells share.
        3,
        "fifty identically formatted cells must share one cellXfs entry:\n{xml}"
    );
    assert_eq!(
        xml.matches("patternType=\"solid\"").count(),
        1,
        "one fill, used fifty times"
    );
    let back = read(&written.bytes).expect("read back");
    assert_eq!(back.sheets[0].cells.len(), 50);
    assert!(back.sheets[0]
        .cells
        .iter()
        .all(|c| c.fill == Some(0xFFFF_FF00)));
}

/// Reads one part out of a written container, for tests that need to look at
/// the XML rather than only at what our own reader makes of it.
fn part_bytes(written: &Written, name: &str) -> Vec<u8> {
    let archive = usk_zip::Archive::open(&written.bytes).expect("our own container must open");
    archive
        .read_named(name)
