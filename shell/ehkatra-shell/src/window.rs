...
pub struct Shell {
    ...
    scrolling: bool,
    /// The caret rectangle last handed to the platform, so the IME cursor area
    /// is set when it moves and not on every frame. `set_ime_cursor_area` is a
    /// Win32/AppKit call and a scroll of a still document must not make one.
...
impl Shell {
    ...
    /// Tells the platform where the caret is, so the IME candidate window
    /// appears under the composing text (docs/33 §IME).
    ///
    /// Logical coordinates, because that is the space the scene is authored in
    /// and winit converts. Called only when the rectangle moves — the platform
    /// call is a real one, and a still frame must not make it.
    fn sync_ime_area(&mut self) {
        let area = self.app.ime_area();
        if area == self.ime_area {
            return;
        }
        self.ime_area = area;
        let (Some(window), Some([x, y, w, h])) = (&self.window, area) else {
            return;
        };
...
impl ApplicationHandler for Shell {
    fn resumed(&mut self, event_loop: &ActiveEventLoop) {
        ...
        // Input methods, on for the life of the window (docs/33 §IME). This is
        // the switch that makes winit run TSF on Windows and NSTextInputClient
        // on macOS and deliver `WindowEvent::Ime` — docs/33's "native
        // composition … never reimplemented", taken literally: the composition
        // logic is the platform's, and the shell's job is to show it and to
        // say where the caret is.
        //
        // Always allowed rather than toggled with the in-cell editor, because a
        // composition *starts* the edit: the keystrokes that begin one are
        // swallowed by the IME, so a window that only enabled IME once an
        // editor was open could never receive the event that would open it.
    ...
    fn window_event(&mut self, event_loop: &ActiveEventLoop, _id: WindowId, event: WindowEvent) {
        match event {
            ...
            // Composition (docs/33 §IME). winit has already done the platform
            // half — this arm only forwards what TSF or NSTextInputClient
            // decided. `Enabled`/`Disabled` are the platform telling us the
            // input method attached or detached and need no reaction: the shell
            // asked for IME once, at window creation, and does not toggle it.
            WindowEvent::Ime(ime) => {
                let outcome = match ime {
                    Ime::Preedit(text, cursor) => self.app.ime_preedit(&text, cursor),
